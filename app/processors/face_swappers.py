...
class FaceSwappers:
    ...
    def preprocess_image_cscs(self, img, face_kps):
        ...
        return (
            torch.unsqueeze(image, 0).contiguous(),
            cropped_image,
        )  # (C, H, W) e (H, W, C)
    ...
    def calc_swapper_latent_reswapper(self, source_embedding):
        """Calculate latent embedding for ReSwapper models (same as InSwapper)"""
        n_e = source_embedding / l2norm(source_embedding)
        latent = n_e.reshape((1, -1))
        latent = np.dot(latent, self.models_processor.emap)
        latent /= np.linalg.norm(latent)
    ...
    def run_reswapper128(self, image, embedding, output):
        """Run ReSwapper 128x128 face swapping"""
        if not self.models_processor.models["ReSwapper128"]:
            self.models_processor.models["ReSwapper128"] = (
                self.models_processor.load_model("ReSwapper128")
            )

        io_binding = self.models_processor.models["ReSwapper128"].io_binding()
        io_binding.bind_input(
            name="target",
            device_type=self.models_processor.device,
            device_id=0,
        ...
        if self.models_processor.device == "cuda":
            ...
            self.models_processor.syncvec.cpu()
        self.models_processor.models["ReSwapper128"].run_with_iobinding(io_binding)

    def run_reswapper256(self, image, embedding, output):
        """Run ReSwapper 256x256 face swapping"""
        if not self.models_processor.models["ReSwapper256"]:
            self.models_processor.models["ReSwapper256"] = (
                self.models_processor.load_model("ReSwapper256")
            )

        io_binding = self.models_processor.models["ReSwapper256"].io_binding()
        io_binding.bind_input(
            name="target",
            device_type=self.models_processor.device,
            device_id=0,
        ...
        if self.models_processor.device == "cuda":
            ...
            self.models_processor.syncvec.cpu()
        self.models_processor.models["ReSwapper256"].run_with_iobinding(io_binding)
